...
def calculate_total_distance(routes, px, py, depot):
    ...
    total_dist = 0
    for route in routes:
        total_dist += calculate_euclidean_distance(px, py, depot, route[0]) # distance from depot to first node
        for i in range(len(route)-1):
            total_dist += calculate_euclidean_distance(px, py, route[i], route[i+1])
        total_dist += calculate_euclidean_distance(px, py, route[-1], depot) # distance from last node to depot
